...
async function createSession(userId: string): Promise<string> {
  if (!sql) throw new Error("Database connection not initialized in createSession");
  await ensureSchema();
...
async function deleteSession(token: string) {
  if (!sql) throw new Error("Database connection not initialized in deleteSession");
  try {
    await ensureSchema();
    await sql`DELETE FROM user_sessions WHERE token = ${token}`;
    const cookieStore = await cookies();
...
export async function login(email: string, password: string): Promise<{ success: boolean; error?: string }> {
  if (!sql) return { success: false, error: "Database connection not initialized" };
  try {
    await ensureSchema();
...
export async function logout() {
  if (!sql) throw new Error("Database connection not initialized in logout");
  try {
    const cookieStore = await cookies();
    const token = cookieStore.get("auth-token")?.value;
